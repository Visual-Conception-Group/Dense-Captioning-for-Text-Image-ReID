...
class CUHKPEDES_DIFF_BLIP_AUG(BaseDataset):
    dataset_dir = 'CUHK-PEDES-DIFF-BLIP'

    def __init__(self, root='', verbose=True):
        super(CUHKPEDES_DIFF_BLIP_AUG, self).__init__()
        self.dataset_dir = op.join(root, self.dataset_dir)
        self.img_dir = self.dataset_dir

        self.anno_path = op.join(self.dataset_dir, 'cuhk_stable_diff_reid_raw_only_train.json')
        ...
        if verbose:
            self.logger.info("=> CUHK-PEDES DIFF BLIP AUG Images and Captions are loaded")
