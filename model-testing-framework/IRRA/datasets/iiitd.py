...
class IIITD(BaseDataset):
    dataset_dir = 'IIITD-20K-Dataset'

    def __init__(self, root='', verbose=True):
        super(IIITD, self).__init__()
        self.dataset_dir = op.join(root, self.dataset_dir)
        self.img_dir = self.dataset_dir
        self.anno_path = op.join(self.dataset_dir, 'IIITD-20K-ICFG-FORMAT.json')
        ...
        if verbose:
            self.logger.info("=> IIITD Images and Captions are loaded")
