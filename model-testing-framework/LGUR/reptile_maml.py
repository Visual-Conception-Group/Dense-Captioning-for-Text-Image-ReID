        ...
        while curr_batch_zuru < max_batches_zuru and curr_batch_icfg < max_batches_icfg:
            ...
            if curr_batch_zuru < max_batches_zuru: 
                ...
                with torch.backends.cudnn.flags(enabled=False):
                    with innerloop_ctx(network, alpha_optimizer, copy_initial_weights=False) as (fmodel, diffopt):
                        #with s
                        zuru_loss = compute_loss(zuru_s_image, zuru_s_label, zuru_s_caption_code, zuru_s_caption_length, zuru_s_caption_mask)[0]
                        diffopt.step(zuru_loss)

                        #with q
                        _zuru_loss, _zuru_id_loss, _zuru_id_loss_dict, _zuru_ranking_loss, \
            ...
            if curr_batch_icfg < max_batches_icfg:
                ...
                with torch.backends.cudnn.flags(enabled=False):
                    with innerloop_ctx(network, alpha_optimizer, copy_initial_weights=False) as (fmodel, diffopt):
                        #with s
                        icfg_loss = compute_loss(icfg_s_image, icfg_s_label, icfg_s_caption_code, icfg_s_caption_length, icfg_s_caption_mask)[0]
                        diffopt.step(icfg_loss)
                        
                        #with q
                        _icfg_loss, _icfg_id_loss, _icfg_id_loss_dict, _icfg_ranking_loss, _icfg_ranking_loss_dict, _icfg_ranking_loss_dict_text, _icfg_ranking_loss_dict_image, _icfg_pred_i2t_local, _icfg_pred_t2i_local = compute_loss(icfg_q_image, icfg_q_label, icfg_q_caption_code, icfg_q_caption_length, icfg_q_caption_mask)
