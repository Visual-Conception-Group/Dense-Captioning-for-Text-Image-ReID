...
if __name__ == '__main__':
    ...
    for epoch in range(opt.start_epoch, opt.epoch):
        ...
        print("max_batches_zuru", max_batches_zuru)
        print("max_batches_icfg", max_batches_icfg)
        ...
        #     max_len = max(max_len, len(train_loaders[i]))
        #     cur_batch.append(0)

        # for i in range(len(train_loaders)):
        #     if cur_batch[i] >= max_len:
        #         exit_epoch = True
        #         break
        # while min(cur_batch[0], cur_batch[1]) < max_len:

        while curr_batch_zuru < max_batches_zuru and curr_batch_icfg < max_batches_icfg:
            isZuru = False
            isIcfg = False
            ...
            if curr_batch_zuru < max_batches_zuru: 
                ...
                with torch.backends.cudnn.flags(enabled=False):
                    with innerloop_ctx(network, alpha_optimizer, copy_initial_weights=False) as (fmodel, diffopt):
                        #with s
                        fmodel, zuru_loss = compute_loss(zuru_s_image, zuru_s_label, zuru_s_caption_code, zuru_s_caption_length, zuru_s_caption_mask, fmodel)[:2]
                        diffopt.step(zuru_loss)

                        #with q
                        fmodel, _zuru_loss, _zuru_id_loss, _zuru_id_loss_dict, _zuru_ranking_loss, \
                            _zuru_ranking_loss_dict, _zuru_ranking_loss_dict_text, _zuru_ranking_loss_dict_image, \
                                _zuru_pred_i2t_local, _zuru_pred_t2i_local = \
                                    compute_loss(zuru_q_image, zuru_q_label, zuru_q_caption_code, zuru_q_caption_length,\
                                        zuru_q_caption_mask, fmodel)
            ...
            if curr_batch_icfg < max_batches_icfg:
                ...
                with torch.backends.cudnn.flags(enabled=False):
                    with innerloop_ctx(network, alpha_optimizer, copy_initial_weights=False) as (fmodel, diffopt):
                        #with s
                        fmodel, icfg_loss = compute_loss(icfg_s_image, icfg_s_label, icfg_s_caption_code, icfg_s_caption_length, icfg_s_caption_mask, fmodel)[:2]
                        diffopt.step(icfg_loss)
                        
                        #with q
                        fmodel, _icfg_loss, _icfg_id_loss, _icfg_id_loss_dict, _icfg_ranking_loss, _icfg_ranking_loss_dict, _icfg_ranking_loss_dict_text, _icfg_ranking_loss_dict_image, _icfg_pred_i2t_local, _icfg_pred_t2i_local = compute_loss(icfg_q_image, icfg_q_label, icfg_q_caption_code, icfg_q_caption_length, icfg_q_caption_mask, fmodel)

            #outer loop computations
            beta_optimizer.zero_grad()
            if isIcfg and isZuru:
                maml_loss = _zuru_loss+_icfg_loss
            elif isZuru:
                maml_loss = _zuru_loss
            elif isIcfg:
                maml_loss = _icfg_loss
            else:
        ...
        # logging.info("Epoch: %d/%d" % (epoch+1, opt.epoch))
