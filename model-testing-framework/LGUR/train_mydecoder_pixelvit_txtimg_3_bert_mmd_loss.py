...
if __name__ == '__main__':
    ...
    opt.ViT_layer = 8
    opt.wordtype = 'bert'
    model_name = 'mmd_loss_validated_correctly'
    # model_name = 'test'
    ...
    for epoch in range(opt.start_epoch, opt.epoch):
        ...
        for times, [image, label, caption_code, caption_length, caption_mask] in enumerate(train_dataloader):
            ...
            # print("Mean across dim 0",torch.mean(image_embedding, dim=1).shape, torch.mean(text_embedding, dim=1).shape)
            # print("Square shape", torch.square(torch.mean(image_embedding, dim=1) - torch.mean(text_embedding, dim=1)).shape)
            # print("Loss", torch.square(torch.mean(image_embedding, dim=1) - torch.mean(text_embedding, dim=1)).sum())

            mmd_loss = torch.square(torch.mean(image_embedding, dim=1) - torch.mean(text_embedding, dim=1)).sum()
            mmd_loss_dict =  torch.square(torch.mean(image_embedding_dict, dim=1) - torch.mean(text_embedding_dict, dim=1)).sum()
            ...
            optimizer.zero_grad()
            loss = (id_loss + ranking_loss + id_loss_dict + ranking_loss_dict + ranking_loss_dict_text + ranking_loss_dict_image + 0.1*(mmd_loss + mmd_loss_dict))
            loss.backward()
            # network.eval()
            # test_best = test_part(opt, epoch + 1, times + 1, network,
            #                       val_img_dataloader, val_txt_dataloader, test_best)
            ...
            # network.train()
            if (times + 1) % 50 == 0:
                logging.info("Epoch: %d/%d Setp: %d, mmd_loss: %.2f, mmd_loss_dict %.2f, ranking_loss: %.2f, id_loss: %.2f, ranking_loss_dict: %.2f, id_loss_dict: %.2f,ranking_loss_dict_text: %.2f, ranking_loss_dict_image: %.2f,"
                             "pred_i2t_local: %.3f pred_t2i_local %.3f"
                      % (epoch+1, opt.epoch, times+1, mmd_loss, mmd_loss_dict, ranking_loss, id_loss, ranking_loss_dict,id_loss_dict,ranking_loss_dict_text,ranking_loss_dict_image,pred_i2t_local, pred_t2i_local))

            ranking_loss_sum += ranking_loss
            id_loss_sum += id_loss
            pred_i2t_local_sum += pred_i2t_local
            pred_t2i_local_sum += pred_t2i_local
            mmd_loss_sum += mmd_loss
        ...
        mmd_loss_avg = mmd_loss_sum / (times + 1)

        logging.info("Epoch: %d/%d , mmd_loss: %.2f, ranking_loss: %.2f, id_loss: %.2f,"
                     " pred_i2t_local: %.3f, pred_t2i_local %.3f "
                     % (epoch+1, opt.epoch, mmd_loss_avg, ranking_loss_avg, id_loss_avg, pred_i2t_local_avg, pred_t2i_local_avg))
