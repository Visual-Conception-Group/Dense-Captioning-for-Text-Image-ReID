...
def parse_args():
    ...
    parser.add_argument('--json_root', default='./ZURU-BLIP-2-ICFG-FORMAT.json', type=str)
    # parser.add_argument('--out_root', default='./processed_data_singledata_ZURU', type=str) # processed_data_spa_img
    parser.add_argument('--out_root', default='./processed_data_singledata_ZURU_BLIP_2', type=str) # processed_data_spa_img
...
def main(args):
    test_json = split_json(args)[1]

    good_vocab = read_dict('./processed_data_singledata_ZURU/ind2word.pkl')
    save_dict(good_vocab, os.path.join(args.out_root, 'ind2word'))
    word2Ind = Word2Index(good_vocab)
