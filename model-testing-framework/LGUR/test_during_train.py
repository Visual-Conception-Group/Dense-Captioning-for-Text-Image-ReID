...
def write_result(similarity, img_labels, txt_labels, txt_img_index, name, txt_root, best_txt_root, epoch, best, iteration, opt=None):
    ...
    i2t_cmc, i2t_map = evaluate(similarity, img_labels, txt_labels)
    ...
    str = "i2t: @R1: {:.4}, @R5: {:.4}, @R10: {:.4}, map: {:.4}".format(i2t_cmc[0], i2t_cmc[4],
                                                                           i2t_cmc[9], i2t_map)
    write_txt(str, txt_root)
    print(str)

    if opt.mode == "test":
        print("Results being appended in CSV")
        with open(f'results_{opt.model_name}.csv', 'a') as f:
            f.write(f"""
{opt.train_dataset}, {opt.dataset},
i2t, _, _, _, t2i, _, _, _,
r1, r5, r10, map, r1, r5, r10, map,
{i2t_cmc[0]}, {i2t_cmc[4]}, {i2t_cmc[9]}, {i2t_map}, {t2i_cmc[0]}, {t2i_cmc[4]}, {t2i_cmc[9]}, {t2i_map}, 
""")
...
def test_part(opt, epoch, iteration, network, img_dataloader, txt_dataloader, best):
    ...
    best = write_result(similarity, img_labels, txt_labels, txt_img_index, 'similarity_all:',
                        txt_root, best_txt_root, epoch, best, iteration, opt)
    return best
