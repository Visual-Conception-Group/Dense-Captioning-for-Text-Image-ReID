...
if __name__ == '__main__':
    ...
    if opt.dataset == 'ICFG-PEDES':
        ...
        opt.vocab_size = 268
        opt.dataroot = '' # expects absolute paths
    elif opt.dataset == 'IIITD_BLIP_LDM_4':
        # opt.pkl_root = '/home/Vibhu/git-stuff/Person-Re-ID/LGUR/processed_data_singledata_IIITD/'
        opt.pkl_root = '/raid/home/vibhu20150/Person-Re-ID/LGUR/processed_data_singledata_IIITD_BLIP_LDM_4/'
        opt.class_num = 15000 
        opt.vocab_size = 425
        opt.dataroot = '' # expects absolute paths
    elif opt.dataset == 'IIITD_BLIP_LDM_5':
        # opt.pkl_root = '/home/Vibhu/git-stuff/Person-Re-ID/LGUR/processed_data_singledata_IIITD/'
        opt.pkl_root = '/raid/home/vibhu20150/Person-Re-ID/LGUR/processed_data_singledata_IIITD_BLIP_LDM_5/'
        opt.class_num = 15000 
        opt.vocab_size = 1037
        opt.dataroot = '' # expects absolute paths
    ...
    opt.mode = 'train'
    opt.dataset = 'IIITD_BLIP_3' #ALSO CHANGE PKL ROOT
    opt.pkl_root = '/raid/home/vibhu20150/Person-Re-ID/LGUR/processed_data_singledata_IIITD_BLIP_3/'
    ...
    param_groups_aug = [{'params': DETR_params, 'lr': opt.detr_lr*0.5},
                    {'params': network.ImageExtract.parameters(), 'lr': opt.lr*0.1*0.5},
                    {'params': network.TextExtract.parameters(), 'lr': opt.lr*0.5},
                    {'params': network.conv_1X1_2.parameters(), 'lr': opt.lr*0.5},
                    {'params': id_loss_fun.parameters(), 'lr': opt.lr*0.5}
                    ]
    ...
    scheduler_aug = optim.lr_scheduler.LinearLR(optimizer_aug, start_factor=1, end_factor=0, total_iters=opt.epoch)
    ...
    for epoch in range(opt.start_epoch, opt.epoch):
        ...
        num_batch_aug = len(train_dataloader_aug)
        ...
        train_iterator_aug = iter(train_dataloader_aug)

        while curr_batch < num_batch and curr_batch_aug < num_batch_aug:
            ...
            if phase < 2:
                ...
                [image, label, caption_code, caption_length, caption_mask] = next(train_iterator_aug)
                phase = 0
                curr_batch_aug += 1
                cur_opt = optimizer_aug

            times = curr_batch + curr_batch_aug - 1
