...
if __name__ == '__main__':
    ...
    opt.dataset = 'reptile_batch_gradual_descent'
    ...
    #     opt.dataroot = '' # expects absolute paths
        ...
        opt.vocab_size = 4750
        opt.dataroot = ''
    elif opt.dataset == 'reptile_batch':
        # opt.pkl_root = '/home/Vibhu/LGUR/processed_data_singledata_ICFG/'
        opt.class_num = 17602
        opt.vocab_size = 4601
        opt.dataroot = ''
    elif opt.dataset == 'reptile_batch_gradual_descent':
        # opt.pkl_root = '/home/Vibhu/LGUR/processed_data_singledata_ICFG/'
        opt.class_num = 17602
        opt.vocab_size = 4601
        opt.dataroot = ''
    elif opt.dataset == 'reptile_batch_16000':
        # opt.pkl_root = '/home/Vibhu/LGUR/processed_data_singledata_ICFG/'
        opt.class_num = 20102
        opt.vocab_size = 4750
        opt.dataroot = ''
    ...
    opt.pkl_root = './processed_data_singledata_ZURU/'
    opt.dataset = 'ZURU'
    zuru_train_dataloader = get_dataloader(opt)
    task_dataloaders.append(zuru_train_dataloader)
    task_iterators.append(iter(zuru_train_dataloader))
    ...
    task_iterators.append(iter(icfg_train_dataloader))
    ...
    innerepochs = 4 # number of epochs of each inner SGD
    outerstepsize0 = 1.0 # stepsize of outer optimization, i.e., meta-optimization
    niterations = 16000 # number of outer updates; each iteration we sample one task and update on it
    # END
    ...
    task_times = [0] * len(task_dataloaders)
    ...
    for iteration in range(niterations):
        ...
        for task_epoch in range(innerepochs):
            ...
            batch_data = next(task_iterators[task_id], None)
            ...
            if batch_data == None:
                ...
                # Epoch of task complete
                task_epochs[task_id] += 1
                times = task_times[0] + task_times[1] - 1 
                task_times[0] = 0
                task_times[1] = 0
                ...
                print(model_name)
                network.eval()
                test_best = test_part(opt, task_epochs[0] + task_epochs[1], times + 1, network,
                ...
                # Refresh iterator
                task_iterators[task_id] = iter(task_dataloaders[task_id])
            ...
            [image,label, caption_code, caption_length, caption_mask] = batch_data
            ...
            if (task_times[0] + task_times[1]) % 50 == 0:
                logging.info("Task ID: %d, Task Epoch: %d/%d Setp: %d, ranking_loss: %.2f, id_loss: %.2f, ranking_loss_dict: %.2f, id_loss_dict: %.2f,ranking_loss_dict_text: %.2f, ranking_loss_dict_image: %.2f,"
                            "pred_i2t_local: %.3f pred_t2i_local %.3f"
                    % (task_id, task_epochs[task_id], opt.epoch, task_times[0] + task_times[1], ranking_loss, id_loss, ranking_loss_dict,id_loss_dict,ranking_loss_dict_text,ranking_loss_dict_image,pred_i2t_local, pred_t2i_local))
        ...
        outerstepsize = outerstepsize0 * (((1 - ( iteration / niterations))+1)/2) # linear schedule
