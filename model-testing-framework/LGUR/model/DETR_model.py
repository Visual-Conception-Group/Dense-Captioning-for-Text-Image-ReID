...
class TextImgPersonReidNet_mydecoder_pixelVit_transTXT_3_bert(nn.Module):
    ...
    def forward(self, image, caption_id, text_mask):
        ...
        image_feature = image_feature.reshape(image_feature.shape[0], image_feature.shape[1], 24, 8)
        image_feature = self.conv_1X1_3(image_feature)
        image_feature = image_feature.reshape(image_feature.shape[0], image_feature.shape[1], -1)
        image_feature = image_feature.permute(0, 2, 1)
        # print("CONV_IMAGE FEATURE SHAPE:", image_feature.shape)
